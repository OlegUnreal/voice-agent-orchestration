...
| Kubernetes deployment | [`k8s/`](k8s/) | Production needs orchestration. Namespace, configmap, secret, deployment (engines + web), service, PVC, HPA. 2-10 replicas for engines, 2 for web. Readiness/liveness probes, PVC for traces, HPA on CPU/memory. Manifests wired, not battle-tested | requires cluster |
| TorchServe model serving | [`serving/`](python/serving/) | Production model serving: batching, GPU allocation, model hot-swapping, metrics. Custom handler for fine-tuned BERT reranker. REST + gRPC APIs. Config: batch_size=8, max_batch_delay=100ms, worker management. Wired, not deployed | `[train]` + GPU |
